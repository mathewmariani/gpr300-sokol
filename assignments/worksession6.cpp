...
void load_island(void)
{
  ...
  // state.scene.sea.model.mesh.vbuf = sg_alloc_buffer();
  // batteries::assets::load_obj({
  //     .buffer_id = state.scene.sea.model.mesh.vbuf,
  //     .mesh = &state.scene.sea.model.mesh,
  //     .path = "assets/island/Sea.obj",
  //     .buffer = SG_RANGE(state.file_buffer),
  // });

  state.scene.island.materials.reserve(8);
  state.scene.sea.materials.reserve(8);
...
#define LOAD_SEA_IMAGE(i, filepath)                                      \
  state.scene.sea.materials[i] = sg_alloc_image();                       \
  batteries::assets::load_img({.image_id = state.scene.sea.materials[i], \
                               .path = filepath,                         \
                               .buffer = SG_RANGE(state.file_buffer)})
  ...
  auto shader_desc = (sg_shader_desc){
      .vs = {
          ...
          .uniform_blocks[0] = {
              ...
              .size = sizeof(vs_island_params_t),
              .uniforms = {
      ...
      .fs = {
          ...
          .uniform_blocks[0] = {
              .layout = SG_UNIFORMLAYOUT_NATIVE,
              .size = sizeof(fs_island_params_t),
              .uniforms = {
...
void init(void)
{
  ...
  state.scene.island.model.transform.scale = glm::vec3(0.5f);
  state.scene.sea.model.transform.scale = glm::vec3(0.5f);
}
...
void frame(void)
{
  ...
  // state.scene.island.model.transform.rotation = glm::rotate(state.scene.island.model.transform.rotation, t, glm::vec3(0.0, 1.0, 0.0));
  state.scene.island.model.transform.scale = glm::vec3(0.05f);

  // initialize uniform data
  const vs_island_params_t vs_params = {
      .view_proj = state.camera.projection() * state.camera.view(),
      .model = state.scene.island.model.transform.matrix(),
  };
  const fs_island_params_t fs_params = {
  ...
  {
    state.object_pass.bind.fs.images[0] = state.scene.island.materials[i];
